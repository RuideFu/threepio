...
- Preserve manual RA calibration only as a legacy/educational offset mechanism, while making LST-derived RA the normal operational source of truth.
...
- legacy/manual calibration offset handling,
...
### 0. Manual RA calibration policy

Manual RA calibration is retained only as a legacy and educational feature.

For the new refractor workflow, manual RA calibration is not required for normal operation because the transient telescope's RA reference is derived directly from local sidereal time. In normal use, the system should compute RA/LST from site location and time, with slow `astropy` re-sync as the astronomical truth source.

Manual calibration may still be useful for classroom demonstrations or legacy workflows, but it should not be treated as a required operator step and should not be allowed to obscure the primary LST-based behavior.
...
### 2. Treat manual calibration as a preserved legacy offset

Do **not** let slow `astropy` re-sync overwrite operator-entered calibration semantics, but also do not make manual RA calibration a required operational step for the new refractor workflow.

Recommended behavior:

- compute astronomical sidereal truth from `astropy`,
- use LST-derived RA as the normal operational source of truth,
- treat manual RA calibration, when used, as a legacy/educational offset from that truth source,
- during slow re-sync, refresh the astronomical anchor but preserve `manual_sidereal_offset_seconds`.

This avoids a bad interaction where a background re-sync silently undoes a manual demonstration or legacy correction, while keeping the production system centered on site/time-derived LST.
...
- Manual RA calibration, when used as a legacy/educational offset, survives background re-sync.
...
- optionally perform legacy/manual RA calibration and confirm the display updates correctly,
